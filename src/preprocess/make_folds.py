...
def get_args():
    ...
    parser.add_argument('--output-test')
    parser.add_argument('--n-fold', type=int, default=5)
    parser.add_argument('--seed', type=int, default=10)
    parser.add_argument('--split-test-holdout', type=bool, default=True)
...
def main():
    ...
    df['fold'] = df.ID.map(folds)

    print(df.head())
    with open(args.output_train, 'wb') as f:
        pickle.dump(df, f)

    print('saved to %s' % args.output_train)
