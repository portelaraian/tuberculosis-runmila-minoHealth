...
_columns = [
    'normal', 'tuberculosis', 'mean_by_study'
]
...
def run(train_df, test_df, gb_type, eps=1e-6):
    ...
    for i_label in range(0,2):
        ...
        left_preds = ['%s_l%d' % (label, i) for i in range(1, 1)]
        right_preds = ['%s_r%d' % (label, i) for i in range(1, 1)]
        ...
        for i_fold in range(n_fold):
            ...
            if gb_type == 'lgb':
                ...
                model = CatBoostClassifier(**_params_catboost, eval_metric='AUC', loss_function='Logloss')
